...
- **`Double_Clap`** (트리거 전용): 박수를 두 번 치면 발동 — 탁-탁! **v4: 오디오+비전 융합.**
  주 신호는 **마이크의 음향 임펄스**다 — 박수는 짧고(<110ms 감쇠) 광대역이며 소음 바닥의
  8배 이상 크다는 특징으로 말소리·키보드·지속 소음과 구분하고, 이런 온셋 두 개가
  0.12~1.0초 간격이면 더블 클랩. 단 **최근 3초 안에 카메라에 손이 보였을 때만 발동**해
  TV·문 소리 같은 외부 임펄스를 차단한다. 비전 경로(손 추적 기반)는 보조로 유지하되
