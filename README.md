...
- **`Double_Clap`** (트리거 전용): 박수를 두 번 치면 발동 — 탁-탁! 세게 친 진짜 박수도,
  살살 친 박수도 인식한다. 세게 친 박수는 접촉 순간 모션블러·양손 겹침으로 손 추적이 끊기는데,
  **빠르게 접근하다 추적이 끊기는 것 자체를 접촉으로 추론**하는 이중 경로로 잡는다.
  빠른 접근 속도 게이트 · 근접 손실만 인정 · 재무장 히스테리시스(0.8) · 깍지 취소 ·
  0.08~1.2초 간격 창으로 오작동을 걸러낸다. `./start.sh --test`로 감지 여부를 확인해볼 것
