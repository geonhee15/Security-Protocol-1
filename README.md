...
- **`Double_Clap`** (트리거 전용): 박수를 두 번 치면 발동 — 탁-탁! 처럼 0.2초 간격의 빠른
  더블클랩도 인식한다(박수를 접촉 순간에 카운트, 사이에 손이 살짝만 벌어져도 재무장).
  빠른 접근 속도 게이트 · 접촉 지속 상한(깍지 취소) · 재무장 히스테리시스 · 0.08~1.2초 간격 창으로
  오작동(천천히 손 모으기, 깍지, 경계 떨림)을 걸러낸다. `./start.sh --test`로 감지 여부를 확인해볼 것
